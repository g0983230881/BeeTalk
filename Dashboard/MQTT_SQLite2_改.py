...
def on_message(client, userdata, msg):
    ...
    if KEY>=2:
        ...
        cursor1 = c.execute("select * from DATA2 where NO={}".format(str(KEY-1)))#抓前一筆及這筆的資料
        r1=cursor1.fetchall()
        cursor2 = c.execute("select * from DATA2 where NO={}".format(str(KEY-2)))#抓前一筆及這筆的資料
        r2=cursor2.fetchall()
        #print(r1)
        #print(len(r1))
        #print(type(r1))
        ...
        #r1=r[0]#當前這筆
        #r2=r[1]#前一筆
        dif=float(r11[5])-float(r22[5])#每筆的第6個項目為重量 算重量差
        print(r11[5],r22[5])
        if(dif>=1):#5秒降了1公斤
